...
# JSON for the ESP32 display.
#
# - Reads the Claude Code OAuth token from the macOS Keychain
#   (service "Claude Code-credentials"); Claude Code itself keeps
#   that token refreshed, this script never writes credentials.
# - Calls https://api.anthropic.com/api/oauth/usage
# - Caches the result for CACHE_TTL seconds
# - Serves GET /usage on 0.0.0.0:8787
#
# Python 3 stdlib only. Run: python3 claude_usage_bridge.py
# ============================================================
...
def read_token():
    ...
    if result.returncode != 0:
        raise RuntimeError(f"Keychain read failed: {result.stderr.strip()}")
    ...
    with _lock:
        ...
        try:
            ...
            try:
                ...
                log("401 from usage API, re-reading Keychain and retrying")
...
def main():
    ...
    log(f"Claude Usage Bridge listening on http://0.0.0.0:{PORT}/usage")
    try:
